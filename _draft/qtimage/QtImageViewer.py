...
class QtImageViewer(QGraphicsView):
    ...
    def updateViewer(self):
        ...
        if len(self.zoomStack) and self.sceneRect().contains(self.zoomStack[-1]):
            self.fitInView(
                self.zoomStack[-1], Qt.IgnoreAspectRatio
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if self.canZoom:
                viewBBox = (
                    self.zoomStack[-1] if len(self.zoomStack) else self.sceneRect()
                )
                selectionBBox = (
                    self.scene.selectionArea().boundingRect().intersected(viewBBox)
                )
