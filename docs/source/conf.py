...
# documentation.
#
html_theme_options = {"prev_next_buttons_location": None, "vcs_pageview_mode": "edit"}
...
man_pages = [(master_doc, "autopew", "autopew Documentation", [author], 1)]
...
def linkcode_resolve(domain, info):
    ...
    startdir = os.path.abspath(os.path.join(dirname(autopew.__file__), ".."))
    fn = relpath(fn, start=startdir).replace(os.path.sep, "/")
    if fn.startswith("autopew/"):
        m = re.match(r"(^.*)\+[\.a-z0-9]+$", autopew.__version__)  # develpment version
        if m:  # link to development version; line numbers could be off by a bit
            return "https://github.com/morganjwilliams/autopew/blob/develop/%s%s" % (
                fn,
                linespec
            )
        else:
