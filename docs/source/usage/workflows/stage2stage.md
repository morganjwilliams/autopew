...
INPUT: 
  * .csv - a list of x,y coordinates with names + more than 3 reference points

FORMAT: 
  * the transform requires column names of 'x' and 'y' and also recognises 'name' as spotnames

OUTPUT: 
  * pos, scancsv or csv file in laser coordinate system with corresponding spotnames
...
points = Pew("Reference Points_source.csv", "Reference Points_dest.csv").load_samples(
    "Samples.csv"
)

# %% VISUALISE THE TRANSFORM ---------------------------------------------------------------
fig = plot_transform(
    points.samples[["x", "y"]].values, points.transformed[["x", "y"]].values
)
